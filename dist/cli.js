...
var CONFIG_DIR = path.join(os.homedir(), ".git-nippo");
...
function getGitLog(options = {}) {
  ...
  let commits = [];
  try {
    const result = (0, import_child_process.spawnSync)("git", args, { encoding: "utf-8" });
...
function isGitRepository() {
  const result = (0, import_child_process.spawnSync)("git", ["rev-parse", "--git-dir"], { encoding: "utf-8", stdio: "pipe" });
  return result.status === 0;
}
function getRepositoryName() {
  const remoteResult = (0, import_child_process.spawnSync)("git", ["remote", "get-url", "origin"], { encoding: "utf-8", stdio: "pipe" });
  if (remoteResult.status === 0 && remoteResult.stdout) {
    const remoteUrl = remoteResult.stdout.trim();
    const match = remoteUrl.match(/\/([^/]+?)(?:\.git)?$/);
    if (match) return match[1];
  }
  const dirResult = (0, import_child_process.spawnSync)("git", ["rev-parse", "--show-toplevel"], { encoding: "utf-8", stdio: "pipe" });
...
// src/formatter.ts
function formatAsMarkdown(result, repoName) {
...
async function generateReportWithGemini(result, repoName, apiKey) {
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
...
program2.name("git-nippo").description("git log\u304B\u3089\u65E5\u5831\u3092\u751F\u6210\u3059\u308BCLI\u30C4\u30FC\u30EB").version("1.0.0").option("-s, --since <date>", "\u5BFE\u8C61\u671F\u9593\u306E\u958B\u59CB\u65E5\u6642", "midnight").option("-u, --until <date>", "\u5BFE\u8C61\u671F\u9593\u306E\u7D42\u4E86\u65E5\u6642", "now").option("-a, --author <name>", "\u7279\u5B9A\u306E\u4F5C\u8005\u306E\u30B3\u30DF\u30C3\u30C8\u306E\u307F").option("--no-ai", "AI\u8981\u7D04\u3092\u7121\u52B9\u5316\uFF08git log\u306E\u307F\u51FA\u529B\uFF09").addHelpText("after", `
\u8A2D\u5B9A:
  ${getConfigPath()}
...
async function main() {
  ...
  if (result.commits.length === 0) {
    console.log(`# \u65E5\u5831 - ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}
`);
...
`);
  ...
  if (config2.geminiApiKey && options.ai) {
    try {
      const report = await generateReportWithGemini(result, repoName, config2.geminiApiKey);
      console.log(report);
    } catch (error) {
